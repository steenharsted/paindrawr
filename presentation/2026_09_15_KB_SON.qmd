...
```{r}
#pdr_example_data[1,] |> pull(pdr_data) |> pdr_add_rgba() |> tibble() |> dplyr::rename(pdr_data= `pdr_add_rgba(pull(pdr_example_data[1, ], pdr_data))`) |> mutate(id = pdr_get_info(pdr_data, ".id"))
```